...
{
    private bool IsIronDisappear_SF = false;
    ...
    BoxCollider ironPlateCollider_SF;
    ...
    void Start()
    {
        ...
        ironPlateCollider_SF = ironPlate_SF.GetComponent<BoxCollider>();
    }
    void Update()
    {
        if (!GameManager.gameManager._gameData.IsSmartFarmOpen_T_C2) // 퍼즐을 완료하면 팜이 열린 상태를 유지한다.
        {
            ...
            if (ironPlateData_SF.IsBite) // 합판에 (관찰하기 상태에서) 물기를 사용하면
            {
                ...
                ironPlateCollider_SF.enabled = false;
                IsIronDisappear_SF = true;
                ...
                if (timer >= 3f)
                {
                    ...
                    if(IsIronDisappear_SF)
                    {
                        ...
                        if(brokenLine2Data_SF.IsBite)
                        {
                            ...
                            brokenLine2Data_SF.IsBite = false;
