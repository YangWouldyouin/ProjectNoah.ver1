...
public class C_Consoles_Center: MonoBehaviour
{
    /* 이 오브젝트와 상호작용 하는 변수들 */
    ...
    void Start()
    {
        ...
        consoleCenteData_CC = consoleCenter_CC.GetComponent<ObjData>();
    ...
    void Update()
    {
        if (consoleCenteData_CC.IsObserve)
        {
            if(boxData_CC.IsUpDown)
            {
                CameraController.cameraController.currentView = consoleCenteData_CC.ObservePlusView; //관찰 뷰 : < 시스템 재가동 버튼> 이 보이는 위치
            ...
            {
                CameraController.cameraController.currentView = consoleCenteData_CC.ObserveView; // 관찰 뷰 : 안 보이는 위치
